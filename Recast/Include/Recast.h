...
struct rcConfig
{
	...
	bool simplificationWithZ;		//HS(TN) 03/04/13 : Calculate distance error with Z (3 dimensions)
	...
	float mergeRegionNormalThreshold;//HS(TN) 14/10/13 : Threshold applied on normal (Z) to fix approximative region merging with stairs
};
...
//HS(TN) 12/04/13 : Don't forget to update this as we added 'RC_UNWALKABLE' flag !!!!!
	unsigned int flags : 3;			// Span flags.
	float normZ;					//HS(TN) 14/10/13 : Keep the normal (Z) ! (to optimize -> unsigned int ?)
//HS(TN) 12/04/13 END TAG
...
enum rcSpanFlags
{
	RC_WALKABLE		= 0x01,
	RC_LEDGE		= 0x02,
//HS(TN) 12/04/13 : New flag for forced unwalkable faces
	RC_UNWALKABLE	= 0x04,
//HS(TN) 12/04/13 END TAG
};
...
void rcMarkWalkableTriangles(const float walkableSlopeAngle,
							 ...
							 const int* tris, int nt, int ntu,		//HS(TN) 12/04/13 : ntu = number of forced unwalkable faces (listed at the end of the array)
							 unsigned char* flags, float* normZ);	//HS(TN) 14/10/13 : keep normals (Z)
...
bool rcBuildRegions(rcCompactHeightfield& chf,
					int borderSize, int minRegionSize, int mergeRegionSize, float normalThreshold);	//HS(TN) 14/10/13 : keep normals (Z)
...
bool rcBuildContours(rcCompactHeightfield& chf,
					 ...
					 rcContourSet& cset, const bool simplifyWithZ=false);	//HS(TN) 03/04/13
